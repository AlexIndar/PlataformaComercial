...
class LogisticaController extends Controller
{
    ...
    public static function getDepartments($token){
        $getDepartments = Http::withToken($token)->get('https://localhost:44384/Distribucion/GetDepartments');
        $departments = json_decode($getDepartments->body());
        return $departments;
    }
    public static function getMunicipios($token){
        $getMunicipios = Http::withToken($token)->get('https://localhost:44384/Distribucion/GetMunicipios');
        $municipios = json_decode($getMunicipios->body());
        return $municipios;
    }
    public static function getClasificadores($token){
        $getClasificadores = Http::withToken($token)->get('https://localhost:44384/Distribucion/GetClasificadores');
        $clasificadores = json_decode($getClasificadores->body());
        return $clasificadores;
    }
    public static function getGuias($token,$data){
        $dataJson = json_decode($data);
        $getGuias = Http::withToken($token)->get('https://localhost:44384/Distribucion/GetGuias?paqueteriaID='.$dataJson->paqueteriaID);
        $guias = json_decode($getGuias->body());
        return $guias;
    }
    public static function getGuia($token, $data){
        $dataJson = json_decode($data);
        $getGuia = Http::withToken($token)->get('https://localhost:44384/Distribucion/GetGuia?numeroGuia='.$dataJson->numeroGuia);
    ...
    {
        $dataJson = json_decode($data);
        $guiaSelected = Http::withToken($token)->get('https://localhost:44384/Distribucion/GuiaSelected?idNumeroGuia='.$dataJson->idNumeroGuia.'&numeroGuia='.$dataJson->numeroGuia.'&importeTotal='.$dataJson->importeTotal);
    ...
    {
        $dataJson = json_decode($data);
        $getAutorizacion = Http::withToken($token)->get('https://localhost:44384/Distribucion/GetAutorizacion?user='.$dataJson->user.'&pass='.$dataJson->password);
    ...
    public static function registroGuia($token, $data)
    {
        $dataJson = json_decode($data);
        // dd($dataJson);
        $registroGuia = Http::withToken($token)->post('https://localhost:44384/Distribucion/RegistroGuia',[
