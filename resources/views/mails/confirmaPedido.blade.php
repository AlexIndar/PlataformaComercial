...
<body>
    ...
    <h4 style="line-height: 10px;">Sucursal: {{ $sucursal }}</h4>
            ...
            @if ($pedido[$x]['regalo'] == 0)
                <tr>
                    @if ($pedido[$x]['tipo'] == 'BO')
                        ...
                        <th style='background-color:#fcbf49; padding: 2px; text-align: center;'></th>
                        @if (count($tranIds) > 0)
                            <th style='background-color:#fcbf49; padding: 2px; text-align: center;'>Descuento:
                                {{ $pedido[$x]['descuento'] }}% Plazo: {{ $pedido[$x]['plazo'] }} Tipo:
                                {{ $pedido[$x]['tipo'] }} Subtotal: ${{ $pedido[$x]['subtotal'] }} Evento:
                                {{ $pedido[$x]['evento'] }} TranID: {{ $tranIds[$x] }}</th>
                        @else
                            <th style='background-color:#fcbf49; padding: 2px; text-align: center;'>Descuento:
                                {{ $pedido[$x]['descuento'] }}% Plazo: {{ $pedido[$x]['plazo'] }} Tipo:
                                {{ $pedido[$x]['tipo'] }} Subtotal: ${{ $pedido[$x]['subtotal'] }} Evento:
                    ...
                    @else
                        ...
                        <th style='background-color:#002868; color:white; padding: 2px; text-align: center;'></th>
                        @if ($tranIds != null)
                            <th style='background-color:#002868; color:white; padding: 2px; text-align: center;'>
                                Descuento: {{ $pedido[$x]['descuento'] }}% Plazo: {{ $pedido[$x]['plazo'] }} Tipo:
                                {{ $pedido[$x]['tipo'] }} Subtotal: ${{ $pedido[$x]['subtotal'] }} Evento:
                                {{ $pedido[$x]['evento'] }} TranID: {{ $tranIds[$x] }}</th>
                        @else
                            <th style='background-color:#002868; color:white; padding: 2px; text-align: center;'>
    ...
    <fieldset class="scheduler-border" style="min-height: 140px !important">
        ...
        <h5><strong>SUBTOTAL:</strong> <span
                style="float: right; text-align: right">${{ $detallesPedido['subtotal'] }}</span></h5>
        <h5><strong>IVA:</strong> <span style="float: right; text-align: right">${{ $detallesPedido['iva'] }}</span>
        </h5>
        <h5><strong>TOTAL:</strong> <span
                style="float: right; text-align: right">${{ $detallesPedido['total'] }}</span></h5>
    </fieldset>
    ...
    <div class="row">
        <div class="col-12 text-center d-flex justify-content-center align-items-center"
            ...
            <h5 class="text-center" style="width: 100%; min-width: 1000px; text-align: center;">
